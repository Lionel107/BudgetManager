...
--   • Clés primaires ENTIÈRES (bigint identity) — le desktop utilise des IDs Long.
--     La v1 est online-first : le serveur génère les IDs, tous les appareils les
--     lisent → suffisant pour la synchro multi-appareils (UUID seulement utile
--     pour un futur mode hors-ligne, non retenu ici).
--     `generated by default` (et non `always`) pour autoriser l'insertion d'un ID
--     explicite lors de la migration des données locales.
--   • Chaque table porte user_id -> auth.users(id), défaut = auth.uid() (rempli
--     automatiquement depuis le JWT à l'insertion). Isolation appliquée par la RLS.
--   • Enums stockés en TEXT + contrainte CHECK.
--   • updated_at auto-maintenu par trigger (synchro incrémentale).
--   • L'ancienne colonne CSV transactions.tags est ABANDONNÉE au profit des tables
--     normalisées tags / transaction_tags.
--
-- Ré-exécutable : supprime d'abord les objets existants (base de dev, sans données).
-- ============================================================================

-- 1. Nettoyage (ordre inverse des dépendances ; CASCADE gère le reste)
...
-- 2. Fonction utilitaire : met à jour updated_at à chaque UPDATE
...
-- ============================================================================
-- TAGS + TRANSACTION_TAGS (relation N–N)
-- ============================================================================
